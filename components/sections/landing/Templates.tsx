    ...
    <section className="relative isolate overflow-hidden bg-[#020308] px-6 py-20 lg:px-8">
      <LazyMotion features={domAnimation}>
        ...
        >
          <m.button
            variants={itemFadeUp}
            className="rounded-full border border-white/10 bg-white/5 px-5 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-slate-200 shadow-[0_10px_40px_rgba(0,0,0,0.35)] backdrop-blur"
          >
          ...
          <div className="space-y-4">
            ...
            <m.p variants={itemFadeUp} className="max-w-3xl text-base text-slate-300/85">
          ...
          <div className="grid w-full gap-6 md:grid-cols-3">
            {cards.map((card, index) => (
              <m.article
                ...
                whileTap={tapPress}
                className="flex flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] shadow-[0_30px_90px_-60px_rgba(0,0,0,0.75)] transition hover:border-primary/30 hover:shadow-[0_30px_90px_-55px_rgba(0,223,255,0.5)]"
              >
                  ...
                  <ul className="space-y-2 text-sm text-slate-200/85">
                    {card.bullets.map((bullet) => (
                      <li key={bullet} className="flex items-start gap-2">
                        <span
                          className="mt-1 h-2 w-2 rounded-full bg-primary shadow-[0_0_12px_-4px_rgba(0,223,255,1)]"
                          aria-hidden
                        />
                        <span>{bullet}</span>
                      </li>
          ...
          <m.div variants={itemFadeUp} className="pt-2">
            <m.a
              whileHover={hoverLift}
              whileTap={tapPress}
              href="/contato"
